...
def calculate_nhia_amount(icd_code, days=1, addons=[], flags=[]):
    ...
    breakdown = [f"Base: {data['name']} = ₦{data['base_tariff']:,}"]

    if days > data["base_days"]:
        extra_days = days - data["base_days"]
        extra_pay = extra_days * data["per_extra_day"]
        amount += extra_pay
    ...
    final_amount = amount
    for f in flags:
        if f["level"] == "RED" and "lab" in f["issue"].lower():
